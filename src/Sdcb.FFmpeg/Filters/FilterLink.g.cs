...
public unsafe partial class FilterLink : SafeHandle
{
    ...
    /// <summary>
    /// <para>Size of the partial buffer to allocate. Must be between min_samples and max_samples.</para>
    /// <see cref="AVFilterLink.partial_buf_size" />
    /// </summary>
    public int PartialBufSize
    {
        get => _ptr->partial_buf_size;
        set => _ptr->partial_buf_size = value;
    }
    
    /// <summary>
    /// <para>Minimum number of samples to filter at once. If filter_frame() is called with fewer samples, it will accumulate them in partial_buf. This field and the related ones must not be changed after filtering has started. If 0, all related fields are ignored.</para>
    ...
    /// <see cref="AVFilterLink.channels" />
    /// </summary>
    public int Channels
    {
        get => _ptr->channels;
